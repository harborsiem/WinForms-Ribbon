...
BeginMainLoop:
        ...
        {
            VT_DECIMAL => GetDecimal(value), //(decimal)value.Anonymous.decVal,
            VT_CY => decimal.FromOACurrency(value.data.cyVal.int64),
            _ => ThrowInvalidCast<decimal>(),
        };

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static decimal GetDecimal(PROPVARIANT value)
        {
            DECIMAL d = value.Anonymous.decVal;
            d.wReserved = 0; //PROPVARIANT vt value must be 0, Debugger have problem
            return (decimal)d;
        }
        ...
        {
            partial struct _Anonymous_e__Struct
            {
                partial struct _Anonymous_e__Union
                {
                    [FieldOffset(0)]
