...
class (mainly UserControl) with MenuAction and ShortcutKeys:

        //after init of MenuActionList (_actionList)
...
namespace WinForms.Actions
{
    ...
    public class ShortCutKeysHandler
    {
        ...
        /// <summary>
        /// Add a class (Control) with an MenuActionList and Shortcuts in MenuAction
        /// </summary>
        /// <param name="actionList"></param>
        public void Add(MenuActionList actionList)
        {
            if (actionList == null)
        ...
        /// <summary>
        /// Remove the MenuActionList
        /// </summary>
        /// <param name="actionList"></param>
        public void Remove(MenuActionList actionList)
        {
            if (actionList == null)
                throw new ArgumentNullException(nameof(actionList));
            for (int i = 0; i < _handlerList.Count; i++)
        ...
        class Handler
        {
            public MenuActionList ActionList { get; private set; }
            public ShortCutKeysDelegate ShortCutKeys { get; private set; }
            private List<MenuAction> _actionListWithShortCuts;

            public Handler(MenuActionList actionList)
            {
                ActionList = actionList;
                _actionListWithShortCuts = new List<MenuAction>();
                MenuActionCollection col = actionList.Actions;
                for (int i = 0; i < col.Count; i++)
                {
                    MenuAction action = col[i];
            ...
            private bool ShortCutKeysMethod(ref Message msg, Keys keys)
            {
                ...
                if (msg.Msg == WM_KEYDOWN)
                {
                    ...
                    {
                        MenuAction action = _actionListWithShortCuts[i];
