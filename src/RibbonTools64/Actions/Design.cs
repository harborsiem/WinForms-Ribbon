...
{
    /// <summary>
    /// <para>TypeConverter for an extender provider which provides an Action property which is one of the MenuAction of an MenuActionCollection</para>
    /// <para>As I've found no way to get access to the associated collection, an MenuAction provides information
    ...
    /// it's possible to get a reference to the MenuActionCollection (which is only needed for filling the list of 
    /// possible values) during the call to ConvertTo.</para>
    /// </summary>
    public class MenuActionConverter : TypeConverter
    {
        /// <summary>
    ...
    /// <summary>
    /// Editor for an MenuActionCollection
    /// </summary>
    internal class MenuActionCollectionEditor : CollectionEditor
    {
        /// <summary>
        /// Constructor
        /// </summary>
        public MenuActionCollectionEditor()
            : base(typeof(MenuActionCollection))
        {
        }
        /// <summary>
        ...
        {
            Debug.Assert(editValue != null && editValue is MenuActionCollection);
            MenuActionCollection coll = (MenuActionCollection)editValue;
            MenuAction[] res = new MenuAction[coll.Count];
            if (coll.Count > 0)
            {
                coll.CopyTo(res, 0);
            }
