...
namespace UIRibbonTools
{
    ...
    abstract partial class TRibbonObject : IDisposable
    {
        ...
        {
            const string ClassFrontPart = "TRibbon"; //@ attention when renaming class names
            string result = GetType().Name;
            if (result.StartsWith(ClassFrontPart))
                result = result.Substring(ClassFrontPart.Length);
            result = "(" + result + ")";
            return result;
        }
